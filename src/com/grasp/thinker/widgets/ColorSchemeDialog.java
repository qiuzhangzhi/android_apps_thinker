...
/**
 * Shows the {@link ColorPanelView} in a new {@link android.app.AlertDialog}.
 * 
...
public class ColorSchemeDialog extends AlertDialog implements
    ...
    private String TAG = "ColorSchemeDialog";

    private final int mCurrentColor;

    private static ArrayList<ColorObserver> mObserverList = new ArrayList<ColorObserver>();
    ...
    public ColorSchemeDialog(final Context context) {
        ...
        mCurrentColor = PreferenceUtils.getInstance(context).getThemeColor(context);
        setUp(mCurrentColor);
    ...
    public void onAttachedToWindow() {
        ...
        ThinkerUtils.removeHardwareAccelerationSupport(mColorPicker);
    }
    ...
    public void notifyObserver(int color){
        if (DEBUG) Log.d(TAG, "notifyObserver"+color);
        for (ColorObserver observer : mObserverList){
